...
stratify::
	...
	refs into separate "stratify" packs. These packs form the "old
	generation" in a Stratified garbage collection scheme: objects
	stratified into base-stratum packs are protected from cruft expiration
	by subsequent `surface-gc` runs.
+
...
resulting objects into an base-stratum pack with an `.base-stratum` sidecar
