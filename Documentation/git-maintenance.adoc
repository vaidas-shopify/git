...
file. On subsequent runs, only objects newer than the last-stratified
commit are walked, making the task incremental.
+
...
`maintenance.stratified.grace-period`. If stratifying is lagging behind,
the task is skipped because the active stratum would still be too
