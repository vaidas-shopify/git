...
+
Anchors are also deduplicated at the OID level within a single run.
If two configured anchors resolve to the same commit (for example,
`refs/heads/main` and a release branch that was just cut from it), only
the first-listed anchor is stratified; the second is recorded as
`skipped/reason=duplicate-oid` in trace2 output. The follower's
surface-gc readiness check is satisfied by the primary's sidecar via an
OID-equivalence fallback, so the dedup is transparent. Once the anchors
diverge in OID, the follower stratifies independently on the next run.
+
...
stratify-prune::
	...
	ref is no longer listed in `maintenance.stratified.anchor`. Only
	the `.base-stratum` and `.keep` sidecars are removed; the `.pack`
	and `.idx` files remain on disk and become regular packs that
...
+
For safety, the task refuses to run when no anchors are configured at
all (which would otherwise demote every base-stratum pack in the
repository). Remove sidecars by hand if that is the intent.
