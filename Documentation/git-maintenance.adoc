...
stratify::
	The `stratify` task stratifies reachable objects from configured anchor
	refs into separate base-stratum packs. These packs form the base
	stratum in a Stratified garbage collection scheme: objects stratified
	into base-stratum packs are protected from cruft expiration by
	subsequent `surface-gc` runs.
+
For each ref listed in `maintenance.stratified.anchor`, the task
resolves the ref, walks its history with `git rev-list --objects`
(bounded by `maintenance.stratified.min-age`), and packs the
resulting objects into a base-stratum pack with a `.base-stratum` sidecar
file. On subsequent runs, the walk is bounded by `^<frontier>` for every
element of the anchor's maximal stratified frontier antichain — typically
a single commit on linear history, but a set of sibling commits when
batching has stratified the two sides of a merge separately — so only
genuinely new objects are walked.
+
The task also validates existing base-stratum packs before creating new
ones: if an anchor ref has been deleted or force-pushed past the
recorded anchor commit, the corresponding pack is demoted to a regular
pack by removing its `.base-stratum` file.
+
The task requires at least one `maintenance.stratified.anchor` to be
configured; it is a no-op otherwise.

consolidate-stratum::
	The `consolidate-stratum` task merges an anchor's accumulated
	base-stratum packs into fewer, larger packs, following the same
	geometric size progression that `git repack --geometric` uses for
	ordinary packs. Each `stratify` run writes one base-stratum pack per
	batch, so an active anchor steadily accumulates small packs;
	consolidating them keeps the number of base-stratum packs bounded
	and avoids slowing every traversal that has to load them.
+
For each configured anchor, the task sorts that anchor's base-stratum
packs by size, computes a geometric split, and merges the packs below
the split into a single base-stratum pack. The merged pack records a
single anchor commit that dominates the merged coverage; when the packs
below the split still straddle an unstratified merge (more than one
frontier commit remains), the group is skipped until `stratify` advances
past the merge.
+
Packs for anchors that are not listed in `maintenance.stratified.anchor`
are left untouched. If no anchors are configured, the task does nothing
and warns that existing base-stratum packs should be retired with
`stratify-prune`.

stratify-prune::
	The `stratify-prune` task demotes base-stratum packs whose anchor
	ref is not listed in `maintenance.stratified.anchor`. Only the
	`.base-stratum` and `.keep` sidecars are removed; the `.pack` and
	`.idx` files remain on disk and become regular packs that
	`geometric-repack` will absorb on its normal cadence.
+
The `stratify` task warns about orphaned anchors on every run but does
not demote them, so a typo or a temporary edit to
`maintenance.stratified.anchor` does not blast away curated packs. The
`stratify-prune` task is the explicit cleanup step.
+
Because it demotes packs, `stratify-prune` is intentionally not part of
any maintenance strategy: it is never run on a schedule or as part of a
bare `git maintenance run`. It runs only when you select it explicitly
with `--task=stratify-prune`, so an empty or misconfigured anchor list
cannot cause an unattended run to retire the whole base stratum. A
repository can opt the task back into scheduled or manual runs by setting
`maintenance.stratify-prune.enabled` (and, for scheduled runs,
`maintenance.stratify-prune.schedule`).
+
If no anchors are configured, the configured set is empty and every
base-stratum pack is considered an orphan, so the task demotes them
all. This makes `stratify-prune` the explicit, idempotent way to wind
stratification down for a repository: unset the anchor config, run the
task, and the next geometric repack folds the freed packs back into
the active stratum.

surface-gc::
	The `surface-gc` task performs lightweight garbage collection that
	only processes unstratified (active stratum) objects. Base-stratum packs
	are kept intact via `--keep-pack`, so the reachability walk and
	repack only cover the active stratum.
+
Before running, the task checks that stratify stratifying has caught up
sufficiently. For each configured anchor ref, the cutoff is
`maintenance.stratified.min-age` plus
`maintenance.stratified.grace-period`; the task asks whether any commit
older than the cutoff is still reachable from the ref tip after
excluding its stratified frontier antichain. If such a commit exists,
the task is skipped because the active stratum would still be too large
for surface-gc to save meaningful work over a full repack.
+
This task is most useful on large repositories where a full `gc` is
too expensive to run frequently. As stratify progressively stratifies more
of the old history, the active stratum shrinks and surface-gc becomes
increasingly cheap.
...
--dry-run::
	Report what `surface-gc` would do without modifying the repository.
	Currently only supported together with `--task=surface-gc`. For each
	configured `maintenance.stratified.anchor`, prints whether stratifying
	is caught up or how far it is lagging (commits older than
	`maintenance.stratified.min-age` plus `maintenance.stratified.grace-period`
	that remain outside the stratified frontier), the frontier antichain
	width, and the resulting `surface-gc` verdict. Combining `--dry-run`
	with any other task, with `--auto`, or with `--schedule` is an error.
