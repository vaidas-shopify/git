...
Stratified GC takes a different approach: stratify objects that are known to
be reachable into long-lived "stratify" packs (the base stratum), then
scope garbage collection to only the unstratified remainder (the young
generation). This bounds GC cost to the size of the active stratum, not
...
....
  ...
  Stratified GC:
    Pin objects reachable from stable refs -> base-stratum pack (old gen)
...
1. Collects all base-stratum packs and passes them as `--keep-pack` to
...
(stratify) packs. If main's history is stratified up to 2 weeks ago, the
