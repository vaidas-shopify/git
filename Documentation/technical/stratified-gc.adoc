...
Base-stratum packs for the same anchor ref form an incremental chain: each
pack is created with `^<last_stratified>` exclusion, so it depends on earlier
packs for objects that were already stratified. If a pack in the middle of
the chain fails validation (e.g., the anchor ref was force-pushed), all
packs for the same anchor ref with equal or later `stratified_timestamp` must
also be demoted. Without cascade demotion, the surviving packs would have
dangling references to objects that are now in the active stratum,
breaking the closed-set property.

Validation processes packs grouped by anchor ref in `stratified_timestamp`
...
After cascade demotion, the stratify task starts stratifying from scratch
for that anchor ref. Surface-gc skips its run until the stratifying frontier
passes the `min-age + grace-period` threshold again; regular geometric
repack handles the active stratum packs in the meantime.

This is cheap: one ancestor check per base-stratum pack, accelerated to
...
reachable from a stratified object is guaranteed to be in some base-stratum
pack, because base-stratum packs are created by `rev-list --objects` which
enumerates the full transitive closure. The `^<last_stratified>` exclusion
