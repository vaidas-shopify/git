...
any pack whose `anchor_commit` is not an ancestor of its ref. Where
the orphan's `anchor_commit` already lies on some surviving anchor's
history, that commit is preserved verbatim. Where it does not, the
sidecar is rewritten with the merge-base of the orphan's
`anchor_commit` and the chosen surviving tip: the merge-base lies on
both histories, and the orphan pack — which covers everything
reachable from its original `anchor_commit` — necessarily covers the
merge-base as well, so the new claim is true. The recorded frontier
date drops to the merge-base's date, which is harmless: future
stratify runs on the new anchor find their own packs' later
`anchor_commits` via `find_stratified_ancestor` and use those as the
rev-list bound instead.

If no surviving anchor has any base-stratum pack of its own, no kept
pack exists whose closure could be broken, so the orphan can simply be
demoted (the historical behavior). Likewise, if no surviving anchor
shares any history with the orphan's `anchor_commit` (no merge-base),
demotion is safe: with no common ancestry there are no shared objects
for surviving packs to be referencing via cross-anchor dedup.
