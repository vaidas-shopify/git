...
next run. `find_stratified_ancestor()` consults this anchor's own
base-stratum packs (if any survived the validation pass) for a `^bound`
on the rev-list, so the walk only repeats the prefix that lost its
pack.
...
reachable from its `anchor_commit` down to the min-age window. The
`^<last_stratified>` exclusion bounds the *new* walk on subsequent
runs, but it never excludes an object from one of this anchor's
packs unless an earlier pack of the *same anchor* already contains
it.
