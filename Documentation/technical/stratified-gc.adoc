...
configured anchor — rather than per anchor. Maintaining the *union* as
the closed set across an anchor retirement is the responsibility of
`stratify-prune`: when an anchor is dropped from configuration, its
packs may exclusively hold objects that other anchors' packs reference
as ancestors. Unlinking the orphan's `.base-stratum` and `.keep`
outright would make those shared ancestors invisible to surface-gc's
`--kept-pack-boundary` walk, which stops at surviving kept packs and
would never reach into a now-regular orphan pack. The next cruft
repack would then misclassify the shared ancestors as cruft and prune
them at expiration. To prevent this, `stratify-prune` *relabels* each
orphan pack's `.base-stratum` sidecar to claim a surviving anchor as
its `anchor_ref`. The pack stays kept, joins the survivor's group, and
is later folded into the survivor's pack(s) by `consolidate-stratum`'s
geometric merge.

The relabel target is chosen per orphan pack (not once per run): the
recorded `anchor_commit` must be reachable from the chosen
...
strategies are tried in order:
  ...
  1. Where the orphan's `anchor_commit` already lies on some surviving
     anchor's history, that commit is preserved verbatim. The frontier
     date is unchanged and the relabel is a no-op for validation.

  2. Where it does not but the orphan shares some commit history with
     a surviving anchor, the sidecar is rewritten with the merge-base
     of the orphan's `anchor_commit` and the chosen surviving tip: the
     merge-base lies on both histories, and the orphan pack — which
     covers everything reachable from its original `anchor_commit` —
     necessarily covers the merge-base as well, so the new claim is
     true. The recorded frontier date drops to the merge-base's date,
     which is harmless: future stratify runs on the new anchor find
     their own packs' later `anchor_commits` via
     `find_stratified_ancestor` and use those as the rev-list bound
     instead.

  3. Where the orphan and the survivors share no commit history at
     all (no merge-base — independently-rooted anchors, an orphan
     branch, a docs/gh-pages branch, a vendored subtree with its own
     root), the sidecar borrows a surviving anchor's own
     `anchor_commit` from one of its existing base-stratum packs. The
     cross-anchor filter (`filter_already_packed_oids()`) operates at
     the OID level, not by commit ancestry: any tree or blob with a
     colliding hash — the empty tree, an identical
     LICENSE/.gitignore/lockfile blob, a `.gitkeep`, and so on — is
     deduplicated even across unrelated histories, so the orphan's
     pack may exclusively hold objects a surviving pack now references
     after that dedup. Borrowing the survivor's own `anchor_commit`
     keeps validation passing (it is by definition an ancestor of the
     survivor's tip) without poisoning `find_stratified_ancestor`'s
     frontier (the survivor's own packs record the same commit).

Only when no surviving anchor has any base-stratum pack of its own is
demotion taken: in that case there is no kept set whose closure could
be broken, and the orphan's objects are reachable from its still-live
ref (or already unreachable, in which case normal GC handles them
correctly).
...
individual packs are not self-contained, but the union is.
`stratify-prune` preserves this invariant when retiring an anchor by
relabeling the orphan pack's sidecar onto a surviving anchor instead
of unlinking it; see "Cross-anchor coverage and the closed-set
property" above for the rationale.
