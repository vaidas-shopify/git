...
maintenance.stratified.grace-period::
	...
	can lag before `surface-gc` skips its run. The task checks that each
	anchor ref's last-stratified commit date is newer than the combined
	threshold of `maintenance.stratified.min-age` plus this grace period.
	If any anchor's stratifying frontier is older than this cutoff, surface-gc
