...
maintenance.stratified.anchor::
	...
	that are protected from cruft expiration. Supports glob patterns
	(e.g. `refs/heads/release/*`). The `stratify` task is a no-op
	if no anchors are configured. No default value.
