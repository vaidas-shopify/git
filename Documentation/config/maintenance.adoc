...
maintenance.stratified.anchor::
	...
	base-stratum packs. Objects reachable from these refs (bounded by
	`maintenance.stratified.min-age`) are stratified into base-stratum packs
	that are protected from cruft expiration. Each value must be an
	exact ref name (e.g. `refs/heads/main`). No default value.
+
The configured set is treated as the desired set of anchors: any
existing base-stratum pack whose recorded anchor is not in the set is
considered orphaned. When the set is empty, `stratify` is a no-op,
`consolidate-stratum` skips (warning that existing base-stratum packs
should be retired via `stratify-prune`), and `stratify-prune` demotes
every base-stratum pack in the repository.

maintenance.stratified.min-age::
	An approxidate string controlling how old a commit must be before
	its reachable objects are stratified. Only objects from commits older
	than this threshold are included in base-stratum packs. This prevents
	stratifying objects that are still in the active working set.
	Defaults to `2.weeks.ago`.

maintenance.stratified.batch-size::
	Soft limit on the number of objects stratified per anchor ref per
	maintenance run. When set to a positive value, the task stops once this
	many objects have been written, at the next commit boundary (so the
	recorded anchors always cover the pack's contents and a follow-up run
	resumes cleanly). A single commit may exceed the limit so the task can
	preserve closure and make progress. A value of `0` (the default) means
	no limit. Common unit suffixes `k`, `m`, and `g` are supported.
+
A base-stratum pack records the maximal antichain of fully-included
commits it covers (one anchor per live branch), so an un-merged fork is
stratified in a single run rather than one commit at a time: the sidecar
simply lists an anchor for each branch and the next run's frontier
excludes them all. Commits whose merge is still newer than
`maintenance.stratified.min-age` remain in the active stratum until the
merge ages past the threshold.

maintenance.stratified.cruft-expiration::
	An approxidate string controlling the cruft expiration threshold
	for the `surface-gc` task. Unreachable objects in the active stratum
...
maintenance.stratified.grace-period::
	An approxidate string controlling how far behind stratify stratifying
	can lag before `surface-gc` skips its run. The cutoff is
	`maintenance.stratified.min-age` plus this grace period; the task asks
	whether any commit older than the cutoff is still reachable from each
	anchor ref's tip after excluding its stratified frontier antichain. If
	such a commit exists, surface-gc is skipped. Defaults to `1.week.ago`.

maintenance.consolidate-stratum.auto::
	This integer config option controls how often the `consolidate-stratum`
	task should be run as part of `git maintenance run --auto`. If zero
	(the default), then the `consolidate-stratum` task will not run with the
	`--auto` option. A negative value will force the task to run every time.
	Otherwise, a positive value implies the task should run when any
	configured anchor has at least that many base-stratum packs.

maintenance.consolidate-stratum.splitfactor::
	The geometric split factor used by the `consolidate-stratum` task when
	deciding which of an anchor's base-stratum packs to merge, analogous to
	the factor passed to `git repack --geometric`. Values below `2` are
	treated as `2`. Defaults to `2`.
