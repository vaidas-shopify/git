...
maintenance.stratified.batch-size::
	Maximum number of objects to stratify per anchor ref per maintenance run.
	When set to a positive value, the task stratifies at most this many objects
	(oldest first) and records the last stratified commit so subsequent runs
	...
	of `0` (the default) means no limit. Common unit suffixes `k`, `m`,
	and `g` are supported.
