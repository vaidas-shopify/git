...
/*
 * Anti-cruft maintenance task: incrementally pin objects reachable from
 * configured anchor refs into anti-cruft packs (the "old generation"
 * in generational GC).
 */
 ...
 */
/*
 * Collect all anchored packs, grouped by anchor_ref.
 * Each string_list entry's util points to an anchored_pack_group.
 * Packs whose .anchored file cannot be loaded are demoted and skipped.
 */
static void collect_anchored_pack_groups(struct repository *r,
...
static void free_anchored_pack_groups(struct string_list *ref_groups)
{
	size_t i;
	...
	for (i = 0; i < ref_groups.nr; i++) {
		...
		for (j = 0; j < group->nr; j++) {
			if (cascade || validate_single_anchored_pack(r, &group->entries[j])) {
				if (!cascade)
					warning(_("anti-cruft: cascade-demoting %"PRIuMAX" remaining "
						  "pack(s) for ref '%s'"),
...
static struct object_id *find_last_pinned_commit(const char *anchor_ref)
{
	...
	repo_for_each_pack(the_repository, p) {
		...
		if (adata.pinned_timestamp > best_ts) {
			free(best);
			best = xmalloc(sizeof(*best));
			oidcpy(best, &adata.anchor_commit);
			best_ts = adata.pinned_timestamp;
		}
		clear_anchored_data(&adata);
	}
...
/*
 * Filter OIDs from rev_list_out that already exist in anchored packs
 * for the given anchor_ref. This prevents creating packs with objects
 * that are already in prior anchored packs, making each pack truly
 * incremental.
 *
					 ...
					 const char *anchor_ref)
{
	...
	/* Collect anchored packs for this anchor ref */
	repo_for_each_pack(the_repository, p) {
		struct anchored_data adata = { 0 };
...
static int maintenance_task_anti_cruft(struct maintenance_run_opts *opts,
				       struct gc_config *cfg UNUSED)
{
	struct repository *r = the_repository;
	...
	if (repo_config_get_string_multi(r, "maintenance.anti-cruft.anchor",
					 &anchors)) {
		if (!opts->quiet)
			fprintf(stderr, _("anti-cruft: skipped, no anchor refs configured\n"));
		return 0;
	}
	...
	for (i = 0; i < anchors->nr; i++) {
		...
		struct object_id *last_pinned = NULL;
		...
		/* Find the last-pinned commit for this anchor */
		last_pinned = find_last_pinned_commit(anchor_ref);
		 ...
		 * last-pinned commit.
		 *
		 * git rev-list --objects --before=<min-age> <tip> [^<last_pinned>]
		 */
		rev_list.git_cmd = 1;
		...
		if (finish_command(&rev_list)) {
			warning(_("anti-cruft: rev-list failed for '%s'"),
				anchor_ref);
		...
		if (!rev_list_out.len) {
			if (!opts->quiet) {
				if (last_pinned)
					fprintf(stderr,
						_("anti-cruft: '%s' already fully pinned at %s\n"),
						anchor_ref, oid_to_hex(last_pinned));
				else
					fprintf(stderr,
		 ...
		 * some objects unpacked but record the commit as pinned,
		 ...
		 * output to use as the anchor commit in the .anchored
		 * sidecar.
		 *
		 ...
		 */
		{
			 ...
			 * represents the actual pinned frontier. Using the
		...
		/*
		 * Filter out objects that already exist in anchored packs
		 * for this ref. This makes each pack truly incremental,
		 * avoiding duplication even when ^<last_pinned> fails to
		 * exclude all previously packed objects (e.g., after cascade
		 * demotion resets the pinned frontier).
		 */
		{
		...
		if (!rev_list_out.len) {
			if (!opts->quiet)
				fprintf(stderr,
					_("anti-cruft: all objects already packed for '%s'\n"),
					anchor_ref);
			strbuf_release(&rev_list_out);
			free(last_pinned);
			continue;
		...
		if (opts->quiet)
			...
			strvec_push(&pack_proc.args, "--no-quiet");
		strvec_pushf(&pack_proc.args, "%s/pack/anchored",
		...
		if (start_command(&pack_proc)) {
			warning(_("anti-cruft: failed to start pack-objects for '%s'"),
				anchor_ref);
			strbuf_release(&rev_list_out);
			free(last_pinned);
			continue;
		...
		if (finish_command(&pack_proc)) {
			warning(_("anti-cruft: pack-objects failed for '%s'"),
				anchor_ref);
		...
		if (new_pack) {
			write_pack_anchored(new_pack, &tip_oid,
					    anchor_ref,
					    (uint32_t)time(NULL));
			new_pack->is_anchored = 1;
		}
...
static int anti_cruft_auto_condition(struct gc_config *cfg UNUSED)
{
	const struct string_list *anchors = NULL;
...
/*
 * Consolidate anti-cruft packs: apply geometric repacking within
 * each anchor ref's pack set to bound the number of anchored packs.
 */
static int maintenance_task_consolidate_anti_cruft(
		...
		struct gc_config *cfg UNUSED)
{
	...
	repo_config_get_int(r, "maintenance.consolidate-anti-cruft.splitfactor",
			    &split_factor);
	if (split_factor < 2)
		split_factor = 2;
	...
	for (i = 0; i < ref_groups.nr; i++) {
		 ...
		 * The merged pack covers the full pinned range, so use the
		 * latest pinned commit and timestamp.
		 */
		for (j = 0; j < group->nr; j++) {
			struct anchored_pack_entry *e = &group->entries[j];
			size_t k;
			int below_split = 0;
			...
			if (below_split && e->pinned_timestamp > best_timestamp) {
				best_timestamp = e->pinned_timestamp;
		...
		if (start_command(&pack_proc)) {
			warning(_("consolidate-anti-cruft: failed to start "
...

		{
			 ...
			 * Do NOT ^-exclude above-split packs: anchored packs
		...
		if (finish_command(&pack_proc) || !pack_hash.len) {
			if (pack_hash.len)
				warning(_("consolidate-anti-cruft: pack-objects "
		...
		/* Write .anchored sidecar for the new merged pack */
		pack_prefix = xstrfmt("%s/pack/anchored-%s",
				      r->objects->sources->path,
				      pack_hash.buf);
		pack_path = xstrfmt("%s.idx", pack_prefix);
		new_pack = add_packed_git(r, pack_path, strlen(pack_path), 1);
		if (new_pack) {
			write_pack_anchored(new_pack, &best_anchor,
...
static int consolidate_anti_cruft_auto_condition(struct gc_config *cfg UNUSED)
{
	struct repository *r = the_repository;
	struct string_list ref_groups = STRING_LIST_INIT_DUP;
	int threshold = 0;
	...
	repo_config_get_int(r, "maintenance.consolidate-anti-cruft.auto",
			    &threshold);
	if (!threshold)
		return 0;
	if (threshold < 0)
 ...
 * unpinned (young generation) objects. Anti-cruft packs are kept
 * intact via --keep-pack, so the reachability walk and repack only
 * cover the young generation.
 */
/*
 * Check whether anti-cruft pinning has caught up sufficiently for
 * scoped-gc to be worthwhile. For each anchor ref, compare the
 * commit date of the last-pinned commit against (now - min-age - grace).
 * If pinning is lagging behind by more than the grace period, the
 * young generation is still too large for scoped-gc to save work.
 */
static int anti_cruft_pinning_caught_up(struct repository *r, int quiet)
{
	const struct string_list *anchors = NULL;
	const char *min_age_str = "2.weeks.ago";
	...
	repo_config_get_string_tmp(r, "maintenance.anti-cruft.min-age",
				   &min_age_str);
	repo_config_get_string_tmp(r, "maintenance.scoped-gc.grace-period",
				   &grace_str);
	...
	for (i = 0; i < anchors->nr; i++) {
		...
		/* Find the most recent pinned commit for this anchor */
		repo_for_each_pack(r, p) {
			struct anchored_data adata = { 0 };
		...
		if (!best_oid) {
			if (!quiet)
				fprintf(stderr,
					_("scoped-gc: anchor '%s' has no pinned commits yet\n"),
					anchor_ref);
			return 0;
		}
		...
		if (!commit || repo_parse_commit(r, commit)) {
			if (!quiet)
				fprintf(stderr,
					_("scoped-gc: cannot parse pinned commit for '%s'\n"),
					anchor_ref);
			return 0;
		}
		...
		if (commit->date < cutoff_ts) {
			if (!quiet)
				fprintf(stderr,
					_("scoped-gc: skipped, pinning for '%s' is lagging "
				  "(pinned up to %s [%s], need commits newer than "
				  "anti-cruft.min-age(%s) + scoped-gc.grace-period(%s))\n"),
				anchor_ref,
...
static int maintenance_task_scoped_gc(struct maintenance_run_opts *opts,
				      struct gc_config *cfg UNUSED)
{
	struct repository *r = the_repository;
	struct child_process child = CHILD_PROCESS_INIT;
	 ...
	 * scoped-gc to be effective. If pinning is lagging behind,
	 ...
	 * scoped-gc is only useful after anti-cruft has pinned objects.
	 */
	if (!have_anchored)
		warning(_("scoped-gc: no anti-cruft packs found; "
			  "this will be equivalent to a full cruft repack"));

	if (run_command(&child))
		return error(_("failed to perform scoped garbage collection"));

	return 0;
}

static int scoped_gc_auto_condition(struct gc_config *cfg UNUSED)
{
	struct packed_git *p;
	int have_anchored = 0;
	int have_young_packs = 0;

	/*
	 * Scoped GC is useful when there are both anti-cruft packs
	 * (old generation) and regular packs (young generation) to
	 * process.
	 */
	repo_for_each_pack(the_repository, p) {
		if (p->is_anchored)
			...
			have_young_packs = 1;
		if (have_anchored && have_young_packs)
			return 1;
	}
...
static const struct maintenance_task tasks[] = {
	...
	[TASK_ANTI_CRUFT] = {
		.name = "anti-cruft",
		.background = maintenance_task_anti_cruft,
		.auto_condition = anti_cruft_auto_condition,
	},
	[TASK_CONSOLIDATE_ANTI_CRUFT] = {
		.name = "consolidate-anti-cruft",
		.background = maintenance_task_consolidate_anti_cruft,
		.auto_condition = consolidate_anti_cruft_auto_condition,
	},
	[TASK_SCOPED_GC] = {
		.name = "scoped-gc",
		.background = maintenance_task_scoped_gc,
		.auto_condition = scoped_gc_auto_condition,
	},
};
